...
extension Container: _ResolverType {
    public func _resolve<Service, Factory>(_ name: String?, option: ServiceKeyOptionType? = nil, invoker: (Factory) -> Service) -> Service? {
        ...
        if let (entry, fromParent) = getEntry(key) as (ServiceEntry<Service>, Bool)? {
            ...
            case .none, .graph:
                resolvedInstance = resolve(entry, key: key, invoker: invoker)
            case .container:
                ...
                if ownEntry.instance == nil {
                    ownEntry.instance = resolve(entry, key: key, invoker: invoker) as Any
                }
                resolvedInstance = ownEntry.instance as? Service
            case .hierarchy:
                if entry.instance == nil {
                    entry.instance = resolve(entry, key: key, invoker: invoker) as Any
                }
...
extension Container: ResolverType {
    ...
    fileprivate func resolve<Service, Factory>(_ entry: ServiceEntry<Service>, key: ServiceKey, invoker: (Factory) -> Service) -> Service {
