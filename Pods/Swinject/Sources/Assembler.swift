...
open class Assembler {
    ...
    open var resolver: ResolverType {
        return container
    }
    ...
    ///
    open func apply(_ assembly: AssemblyType) {
        run([assembly])
    }
    ...
    ///
    open func apply(_ assemblies: [AssemblyType]) {
        run(assemblies)
    }
    
    // MARK: Private
    
    fileprivate func run(_ assemblies: [AssemblyType]) {
