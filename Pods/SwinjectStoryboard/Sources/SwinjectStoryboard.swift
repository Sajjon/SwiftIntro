...
open class SwinjectStoryboard: _SwinjectStoryboardBase, SwinjectStoryboardType {
    ...
    open static var defaultContainer = Container()
    ...
    open override func instantiateViewController(withIdentifier identifier: String) -> UIViewController {
    ...
    fileprivate func injectDependency(to viewController: UIViewController) {
        ...
        if let container = container.value as? _ResolverType {
            ...
            let _ = container._resolve(registrationName, option: option) { (factory: FactoryType) in factory(self.container.value, viewController) }
        } else {
